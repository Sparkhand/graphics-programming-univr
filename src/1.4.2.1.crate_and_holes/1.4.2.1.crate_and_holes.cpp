/******************************************************************************
 * File:        1.4.2.1.crate_and_holes.cpp
 * Author:      Davide Tarpini (https://github.com/Sparkhand)
 * Description: This is an example of how to load two textures one above the
 *              other. The first texture is a crate and the second one is a
 *              bunch of bullet holes.
 * Notes:       Props to Mathias Dalla Palma
 *              (https://github.com/mathiasdallapalma) for proposing
 *              this example.
 *****************************************************************************/
...
const struct WINDOW_PROPS
{
    ...
    // Window title
    const char* TITLE = "LearnOpenGL - 1.4.2.1 - Crate and Holes";
    // Clear color
...
int main()
{
    ...
    Shader ourShader("1.4.2.1.crate_and_holes.vs", "1.4.2.1.crate_and_holes.fs");
    ...
    // Load and create texture 1 (crate)
    unsigned int textureCrate;
    try
    {
        textureCrate = loadTexture("resources/textures/crate.jpg");
    }
    catch (const std::exception& e)
    {
        std::cerr << e.what() << std::endl;
    ...
    unsigned int textureHoles;
    try
    {
        textureHoles = loadTexture("resources/textures/holes.png", true);
    }
    catch (const std::exception& e)
    {
        std::cerr << e.what() << std::endl;
    ...
    ourShader.use();
    ourShader.setInt("textureCrate", 0);
    ourShader.setInt("textureHoles", 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindTexture(GL_TEXTURE_2D, textureHoles);
...
unsigned int loadTexture(std::string path, bool transparency, bool flip)
{
    ...
    if (data)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, transparencyMode, width, height, 0, transparencyMode,
